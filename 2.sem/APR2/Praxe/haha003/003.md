...
class Library: def **init**(self): self.shelves = {}
    ...
    def __iter__(self):
        self._shelf_iter = iter(self.shelves.values())
        self._book_iter = None
        return self
    def __next__(self):
        if self._book_iter is not None:
            try:
                return next(self._book_iter)
            except StopIteration:
                self._book_iter = None
        shelf = next(self._shelf_iter)  # May raise StopIteration
        self._book_iter = iter(shelf.books)
        return next(self._book_iter)
    ...
    def find_by_name(self, name_part):
        results = []
        for shelf in self.shelves.values():
            results.extend(shelf.search_books(title=name_part))
        return results
